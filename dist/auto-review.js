...
async function generateReviewWithAI(productTitle, productDesc, usedDb) {
...
async function openProfileSafely(omni, profileId, options) {
    try {
...
async function main() {
    ...
    try {
        ...
        productUrls = (await page.evaluate(() => {
            ...
            const flashSaleContainer = titleContainers.find(el => el.textContent.trim().toLowerCase().includes('flash sale'));
            ...
            let current = flashSaleContainer;
            let productRow = null;
            while (current) {
