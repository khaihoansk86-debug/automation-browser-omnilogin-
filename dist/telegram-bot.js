...
function parseDelayRange(value, defaultVal) {
    if (!value)
...
function getLocalAiAppScriptPath(appId) {
...
async function runAiAppForProfiles(telegram, chatId, omni, app, profileRefs, delayRange, profileRunSeconds, closeAfterRun, mktProxyConfig, gscSyncConfig, state) {
    ...
    try {
        const queueStartMsg = await telegram.sendMessage(chatId, [
        ...
        if (gscSyncConfig.enabled && gscSyncConfig.syncBeforeRun && app.appId === DEFAULT_APP_ID) {
            ...
            await telegram.editMessageText(chatId, queueStartMsg.message_id, [
                `🚀 <b>KHỞI CHẠY HÀNG ĐỢI: [${app.alias}]</b>`,
                `--------------------------------------------`,
                `📋 Tổng số profile: <b>${profileRefs.length}</b>`,
                `👤 Danh sách: <code>${profileRefs.map((p) => p.name).join(', ')}</code>`,
        ...
        for (let index = 0; index < profileRefs.length; index++) {
            ...
            if (!result) {
                throw new Error(`Không thể chạy kịch bản ${app.appId}: không tìm thấy local script và AI App không phản hồi`);
            }
            if (!hasLocalScript) {
                if (!result.ok) {
                    await telegram.sendMessage(chatId, `❌ <b>Profile ${profileName} (ID: ${profileId}) chạy lỗi:</b> ${code(result.error || 'không rõ lỗi')}`);
                }
                else {
                    await telegram.sendMessage(chatId, `🟢 <b>Profile ${profileName} (ID: ${profileId}) đã gửi lệnh chạy thành công.</b>`);
                }
            }
                ...
                if (waitResult === 'captcha') {
                    if (index < profileRefs.length - 1) {
                        ...
                        if (actualDelay > 0) {
                            ...
                            const captchaDelayMsg = await telegram.sendMessage(chatId, `⏳ <b>Tạm nghỉ sau CAPTCHA:</b> Đang nghỉ ${actualDelay}s trước khi chuyển sang profile tiếp theo...`);
            ...
            if (index < profileRefs.length - 1) {
                ...
                if (actualDelay > 0) {
                    ...
                    const delayMsg = await telegram.sendMessage(chatId, `⏳ <b>Tạm nghỉ:</b> Đang nghỉ ${actualDelay}s trước khi chạy Profile <b>${profileRefs[index + 1].name}</b>...`);
                    await delay(actualDelay * 1000);
...
async function runLocalAiAppScript(telegram, chatId, omni, app, profileId, profileName) {
    ...
    function renderStatus(currentStep, detail) {
        if (app.appId === 'profile-warmup-random') {
            return [
    ...
    try {
        ...
        if (app.appId === 'khaihoan-derma-rank-qa') {
            ...
            if (output) {
                ...
                const taskSummaryLines = actions.map((act) => {
            ...
            if (output) {
                ...
                if (output.quotaExceeded) {
                    ...
                    reportText = [
                        ...
                        Array.isArray(output.unindexedUrls) && output.unindexedUrls.length > 0
                            ? output.unindexedUrls.map((u) => `• <code>${escapeHtml(u)}</code>`).join('\n')
                            : 'Không còn URL nào.',
                        `--------------------------------------------`,
                ...
                if (shouldSendFile) {
                    ...
                    if (existsSync(txtFilePath)) {
                        try {
                            await telegram.sendDocument(chatId, txtFilePath, `Danh sách các URL chưa được index còn lại (Profile ${profileName})`);
                        }
                        catch (err) {
            ...
            for (const update of updates) {
                ...
                if (text.startsWith('/review') || text === 'tự động đánh giá sản phẩm derma' || text.startsWith('/danhgia')) {
                    ...
                    try {
                        ...
                        let lastStatusLines = ['Đang khởi chạy kịch bản...'];
                        ...
                        await telegram.editMessageText(chatId, statusMsg.message_id, `❌ <b>Lỗi khi chạy kịch bản:</b> ${code(errMsg)}`).catch(() => { });
                    }
                    continue;
                }
